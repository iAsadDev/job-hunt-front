...
const JobEditForm = () => {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const res = await axios.get(`https://job-hunt-backend-production.up.railway.app/api/jobs/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    setUpdating(true);
    try {
      await axios.put(`https://job-hunt-backend-production.up.railway.app/api/jobs/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      navigate("/my-jobs");
    } catch (err) {
